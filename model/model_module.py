...
class LinearTransformerLabellingBase(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        val_acc = None
        val_f1 = None
        val_loss_mean = torch.stack([out['val_loss'] for out in self.validation_step_outputs]).mean()
        
        if self.cfgs.DATASET not in self.cfgs.BIO_SCHEME:
            val_acc = torch.cat([out['val_acc'] for out in self.validation_step_outputs]).view(-1)
        ...
        self.log_dict(values) #val_loss_mean
    ...
    def optimizer_step(self, current_epoch, batch_nb, optimizer, closure, on_tpu=False,
...
class TwoPassLabelling(pl.LightningModule):
    ...
    def on_test_epoch_end(self):
        test_acc = None
        test_f1 = None
        test_loss_mean = torch.stack([out['test_loss'] for out in self.test_step_outputs]).mean()

        if self.cfgs.DATASET not in self.cfgs.BIO_SCHEME:
            test_acc = torch.cat([out['test_acc'] for out in self.test_step_outputs]).view(-1)
            test_acc = torch.sum(test_acc).item()/(len(test_acc) * 1.0)
        else:
            # Compute F-score
            preds = []
