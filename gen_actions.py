...
class ActionSeq(object):
    ...
    def gen_actions(self):
        ...
        for idx in range(data_size):
            ...
            ctr += len(actions)
            #print(f"Sequence length: {seq_len}, Action length: {len(actions)}")
    ...
    def gen_datasets(self, split='train_only'):
        ...
        with open(os.path.join(self.result_path, dataset_id + '_' + split + '_actions'), 'w') as f:
            ...
            for i in range(len(sentence_list)):
                print(f"Sequence length: {len(sentence_list[i])}, Tag length: {len(tag_list[i])}, Action length: {len(self.results['actions'][i])}")
                ctr2 += len(sentence_list[i])
                ctr3 += len(tag_list[i])
                ctr4 += len(self.results['actions'][i])


            for idx, (seq_iter, tag_iter) in enumerate(zip(sentence_list, tag_list)):
                #print(f"Processing sequence {idx}")
                #print(f"Sequence length: {len(seq_iter)}, Tag length: {len(tag_iter)}, Action length: {len(self.results['actions'][idx])}")
        
                actions_iter = self.results['actions'][idx].tolist()
                """
                if len(seq_iter) != len(tag_iter) or len(seq_iter) != len(actions_iter):
                    print(f"Length mismatch at sequence {idx}: seq_len={len(seq_iter)}, tag_len={len(tag_iter)}, action_len={len(actions_iter)}")
                    print(f"Sequence: {seq_iter}")
                    print(f"Tags: {tag_iter}")
                    print(f"Actions: {actions_iter}")
                    continue
                """
            ...
            print("DATASET # OF TOKENS")
            print(ctr)
            print("DATASET # OF TOKENS - SENTENCE")
            print(ctr2)
            print("DATASET # OF TOKENS - TAG")
            print(ctr3)
            print("DATASET # OF TOKENS - ACTIONS")
            print(ctr4)
